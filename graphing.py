...
from feature_extraction import extract_features, create_text_without_mentions
...
trump_tweets = create_text_without_mentions(trump_tweets)
chump_tweets = create_text_without_mentions(chump_tweets)

# feature extraction
trump_features = extract_features(trump_tweets, drop_length=False, drop_retweets_and_favorites=False)
chump_features = extract_features(chump_tweets, drop_length=False, drop_retweets_and_favorites=False)


# requires trump_tweets_full.json from Google Drive
def tweet_length_boxplots():
    trump_created = cleanse_data(pd.read_json("Datasets/Full/trump_tweets_full.json"), include_created_at=True)
    trump_280 = trump_created[trump_created['created_at'] > '2017-11-07 00:00:00']
    trump_280 = cleanse_data(trump_280)
    trump_280 = create_text_without_mentions(trump_280)
    trump_280_features = extract_features(trump_280, False)

    plt.title("Tweet Length")
    plt.xlabel("Tweet Length")

    plt.boxplot(x=[trump_features['length'], trump_280_features['length'], chump_features['length']],
                vert=False, labels=["Trump", "Trump 280", "Chump"], showfliers=False)
...
def average_word_length_boxplots():
    ...
    plt.boxplot(x=[trump_features['avg_word_length'], chump_features['avg_word_length']], vert=False,
                labels=["Trump", "Chump"], showfliers=False)
...
def number_of_punctuation_characters_boxplots():
    ...
    plt.boxplot(x=[trump_features['no_punctuation'], chump_features['no_punctuation']], vert=False,
                labels=["Trump", "Chump"], showfliers=False)
...
def mean_punctuation_count_bar_chart():
    ...
    plt.bar(x=np.arange(len(trump_punctuation_columns)) - width / 2, width=width,
            height=bar_heights(trump_features, trump_punctuation_columns), color='red', log=True, label='Trump')

    chump_punctuation_columns = list(filter(lambda punctuation: punctuation in chump_features, string.punctuation))
    plt.bar(x=np.arange(len(chump_punctuation_columns)) + width / 2, width=width,
            height=bar_heights(chump_features, chump_punctuation_columns), tick_label=chump_punctuation_columns,
            color='blue', log=True, label='Chump')
...
def mean_favourite_and_retweet_count_bar_chart():
    ...
    plt.bar(x=np.arange(len(column_labels)) - width / 2, width=width, height=bar_heights(trump_features, column_labels),
            color='red', label='Trump')
    plt.bar(x=np.arange(len(column_labels)) + width / 2, width=width, height=bar_heights(chump_features, column_labels),
            tick_label=column_labels, color='blue', label='Chump')
